...
**一切开发任务从 `/sdd <目标>` 开始** —— 新需求、bug、优化、改一句文案都走它。
它先分诊（feature / fix / refactor / chore）再选路径，不让 bug 走一遍 PRD 流程；
闸门之外自主跑，只在**需求澄清、方案定稿、验收交付**三处停（`go` / `auto` 可减档）。
方法论在全局 `sdd-*` skills，本项目口径在 `openspec/config.yaml`（按工序注入，不占常驻）。

**`docs/plans/` 停止新增**：历史文档原样保留，新任务的需求/设计/规格/任务一律走
`openspec/changes/<slug>/`。起因是那种 1500 行含实现代码的计划把同一段代码付费生成三次
（计划里、派工单里、实现时各一次），而只有最后一次进得了仓库。

**工作流自进化**：任何任务中若撞到 ①skill 写的做法走不通 ②同一个坑第二次踩
③某工序产出没人用 ④闸门问错了 ⑤实测与预期差一倍以上 —— 记一行到
全局 `sdd-evolve/FRICTION.md`，并在交付时写成提议**等我确认**（判定细则读 `Skill(sdd-evolve)`）。
**绝不自行修改工作流**；没撞到就零成本跳过，不为凑数硬找。
...
> **产物仓库（`artifact-source: uploaded`）为什么并进 #7 而不是单列一条**：按「少了就不能用、
> 错了就出严重故障」的判定，它少了平台照常用 —— 裸机部署根本不碰这条路；但**错了会出静默
> 错误**（拿错 buildId 的产物 → 行号错位却看起来完全正常），而那恰恰就是 #7 在守的东西，
> 只是版本对不齐的来源从「源码漂移」换成了「产物取错」。单列会让清单按实现方式而不是
> 按能力分类。
>
...
→ 跑 10 套 E2E + 1 套真实 Chrome 前端验收（`ui_verify.js`，排在最后）。
...
`llvm-profdata-tool` / `llvm-cov-tool` 这五个工具链路径，加上产物仓库的
`artifact-root` / `artifact-keep` / `artifact-max-unzipped-bytes` / `artifact-max-entries`，
始终读 yml —— 它们是部署机器的属性（磁盘是机器的），与项目无关。
**但 `artifact-source` 是项目级的**（在库里那份 JSON 里）：同一个平台上，
一个项目跑在裸机、另一个跑在容器里，各自选各自的产物取法。
...
平台要靠被测服务的**编译产物**才解得出行号：Java 要 `.class`、C++ 要 `.gcno`、
Rust 要产物本身（Go 不用，它的覆盖数据自包含）。裸机部署时这些就在本机路径上，
配置里直接填路径即可；**容器化之后平台够不着容器的文件系统**，所以改由 CI
在构建后把产物推给平台。

| 取法 | `artifact-source` | 产物从哪来 |
|---|---|---|
| 裸机（默认） | `local` | `classes-dir` / `cpp-objects-dir` / `rust-binary` 这几条本地路径 |
| 容器 | `uploaded` | 按 buildId 从产物仓库取，CI 用 `POST /api/artifacts/<buildId>?project=<id>&lang=<java\|cpp\|rust>` 推上来 |

**索引键是 buildId，也就是实例自报的那个 sessionid / `COVERAGE_BUILD_ID`**，
两端天然对齐、不需要新的版本标识。按固定路径存的话，推上去的是 commit A 的产物、
容器里跑的是 commit B，平台会拿 A 的字节码解 B 的探针数据 ——
算出**行号错位却看起来完全正常**的报告，正是本平台最要消灭的那种错。

四条由此而来的硬约定：

- **不收 `-dirty` 的 buildId**（上传侧拒、读取侧也拒）。工作树脏时同一个 commit
  能对应无数份不同的产物，取出来的可能不是这个容器加载的那一份
- **取不到产物一律拒绝出报告**，不降级、不跳过那门语言 —— 跳过的话界面上表现为
  「这些代码没被调用过」，与真相完全相反，而且看不出是缺产物
- **只要这一轮真有实例的那几种语言**。C++ 两台都掉线时不该因为「缺 cpp 产物」
  把整轮打成 ANALYZE_ERROR，那会把「少几台降级成 PARTIAL、照常出报告」
  变成「完全没有报告」
- 只保留最近 `artifact-keep` 个构建（默认 10，**至少 1**），存新的时顺手清旧的

**已知缺口：`uploaded` 项目目前只能经 API 建，页面上的新建向导建不出来。**
向导第 6 步的自检不看 `artifact-source`，仍要求三条本地产物路径真实存在，
而 uploaded 项目按设计根本不需要它们 —— 自检过不了，创建按钮就是灰的。
页面加 `artifact-source` 这个选项时要一并修它。

**安全边界：上传接口能写平台磁盘，因此仍假设部署在内网。** buildId 与 project
都直接参与磁盘路径，两者各有各的校验：buildId 限死 40 位小写 hex，
project **做包含性检查**（归一化后必须仍落在产物根之下 —— 它由用户自取，
现网已有中文项目名，限字符集会把合法项目挡在外面）；zip 条目另挡了 Zip Slip。

解压膨胀另有两道闸，因为上传包本身的 200MB 上限拦不住它 ——
压着几十 KB、解开能把磁盘写满，而平台的磁盘是共享的（趋势入库、gcov 临时文件、
日志全要写盘）：`artifact-max-unzipped-bytes`（默认 1 GiB）挡「少数巨大条目」，
`artifact-max-entries`（默认 10 万）挡「海量空条目」—— 后者总字节几乎为零，
字节闸一点都拦不住。两道都是**边写边判**：撞上上限时这一批字节还没落盘，
而不是写完整个条目再报错（那样一个足够大的条目在被发现之前就已经把磁盘写满了）。
撞闸回 **400 不是 5xx**：CI 据此知道该换包，而不是把同一个包一遍遍重推。

**剩下这条没堵**，暴露到公网前必须先补上：

- **没有鉴权** —— 任何能访问平台的人都能推产物、删产物
...
4. **前提不成立时，断言必须报「判不了」，不能照判。**多实例聚合那条
   ...
   **正确的做法是先把前提做实**：取不齐就重试几次（重试的是<b>取数</b>，不是断言本身），
   ...
   > 2026-09-06 记：根因已查明并修复 —— JaCoCo tcpserver 的 accept backlog 只有 1，
   > 被测 JVM 繁忙时上一次 dump 的连接未收尾，下一次的 SYN 被 RST（`Connection refused`）；
   > `ProbeClient` 已改为被拒时重试一次（**只重试被拒，不重试超时** —— 超时是真掉线的
   > 信号，重试它会让单轮采集最坏耗时翻倍）。此前 16 次定向复现全部失败，是因为都在
   > **空闲态**打；加上负载后 1/20 次调用即命中。
